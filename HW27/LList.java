...
{ 
  ...
  private class MyIterator implements Iterator<T> 
  {
    ...
    public MyIterator() 
    {
      _dummy = new DLLNode("ech", null, _head); //just a dummy string node
      _okToRemove = false; //is this line really necessary since when instantiating a bool, the default is false?
    }
    ...
    public void remove() 
    {
      ...
      if ( ! _okToRemove )
        ...
        _dummy.getNext.setPrev(null); //removes current reference to this node from the LList
	    }
	    //if ________...
	    else if (_dummy.getNext == null) {
        _dummy.getPrev.setNext(null); //removes current reference to this node from the LList
	    }
      //
      //
      // Q: How many ELSE-IF's are necessary? 
      // Q: What are the cases you must consider?
      //    Here, only three, since the node to be removed may be either the first/head node, the last/tail node or anywhere in between.
      //    And since the procedure for each is different, each requires a conditional.
      //
      //      
	    //if ________...
	    else {
        _dummy.getPrev.setNext(_dummy.getNext());
        _dummy.getNext.setPrev(_dummy.getPrev()); //should work since dummy should still have a ref to its pref before dereferincing it
      }
